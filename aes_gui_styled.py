...
ERRORS = {
	...
	'[F_UC-0]':"Please check back later or contact with Satyam Mishra. Probably there is an update on the way!",
...
'''
Satyam Mishra (Github: https://github.com/satyammishra82 ), India
'''
...
class UI(QMainWindow):
	...
	def __init__(self):
		...
		self.setWindowTitle(f'ADVANCED FILE SECURITY SYSTEM V{str(EX_Version)}') # Window Title and Version Number It Will Provide By master Dev.
		self.setWindowIcon(QIcon('images/Small_Logo.png')) # Window icon

		### Default variables ###
		self.DATABASE_FILE = None # Default key/nonce database
		self.FILE_EXT = '.encex'
		self.FEEDBACKURL = 'https://forms.gle/FHCfRAtRj83MoyjU6' #Google From will collect Feedback from Customer.
	...
	def LabelSwitcher(self, label_name):
		if label_name == 'enc_key_label':
			if self.enc_key_label.text() == 'Key (B):':
				...
				self.enc_key_input.setPlaceholderText('Type your key in Bytes format (example: Satyam123)')
			...
			if self.dec_key_label.text() == 'Key (B):':
				...
				self.dec_key_input.setPlaceholderText('Type your key in Bytes format (example: Satyam123)')
